...
**Etapa atual:** **E3 — paridade (portão P1), lado Claude PRONTO.** E1 e E2 concluídas (2026-07-16; ver parágrafo abaixo). O comparador `scripts/e03_paridade.py` está escrito e testado em golden sintético (aprova golden idêntico ao pipeline, reprova valor adulterado além do C1 — 36 testes verdes no total). **Só falta o insumo 👤:** rodar o `ExportGoldenIFM.mq5` na conta MetaQuotes-Demo e copiar os 5 `golden_*.csv` para `data/raw/` (ver pendências).

**Fechamento anterior (2026-07-16, Rhuan):** reexport v2.00 aprovado no inventário (cobertura 28/28 nos 8 TFs, zero linhas inválidas); parquet de métricas M30–D1 + W1/MN gerado (hash de config `2bbc928fa01e`; abertos+selados cortados fisicamente: M30 59065+9245, H1 29539+4624, H4 7395+1159, D1 1235+194, W1 508+40, MN1 116+10, zMov 59327+9265).
...
- **Rodar o `ExportGoldenIFM.mq5`** (`tools/export_golden/`, instruções no README de lá): compilar (F7), rodar na conta MetaQuotes-Demo com os defaults, copiar os 5 `golden_*.csv` de `MQL5\Files\IFM_golden\` para `data/raw/`. Depois: `python scripts/e03_paridade.py` gera o `results/E03_paridade.md` no checklist C1 e o P1 pode ser carimbado. *(Verificado em 2026-07-16: o MT5 do Léo não tem os arquivos — a pendência segue aberta para quem tiver a conta MetaQuotes-Demo à mão.)*
...
| 2026-07-15 | E2 (pipeline + testes — Léo) | Cadeia completa do painel em Python (`scripts/ifm_metrics/`: IFM Light, S, cesta, vel/acel/zvel, zS, mtf/VETO/candidata/rank H1, zMov/zHist) + orquestrador `e02_gerar_metricas.py` (cache por hash, trava de proveniência, corte físico do selado já no estágio de métricas). 34 testes verdes: fixtures à mão + vetorizado × porta de referência (tradução literal do MQL5, `reference.py`) + ponta a ponta em raw sintético. Parquet real aguarda a decisão de dados. | 4027ef5 |
...
| 2026-07-16 | E3-preparo (Claude/Rhuan) | golden_*.csv antigos descartados como verdade C1 (origem desconhecida — decisão de Rhuan; quarentena em legadoV2_*). Criada a ferramenta oficial de replay `tools/export_golden/ExportGoldenIFM.mq5` (cópia literal do cálculo do IFM v1.0 + golden_meta com proveniência completa). Pendência 👤: rodar no MT5. | 54d6a3f |
| 2026-07-16 | E3-comparador (Léo) | Comparador de paridade `scripts/e03_paridade.py`: valida golden_meta contra o config (parâmetros e conta — paridade contra a versão certa), compara os 4 golden (pares/strength/derivadas/cross, incl. zMov/zHist e âncora por tempo via asof), emite `results/E03_paridade.md` no checklist C1 com os maiores desvios explicados. Testado em golden sintético fabricado do próprio pipeline: aprova idêntico, reprova valor adulterado (36 testes verdes). Verificado: MT5 do Léo não tem os golden — pendência 👤 segue. | (este commit) |
