...
- **Versão principal:** v1.1 (`src/IFM.mq5`, ~2040 linhas) — corrige o look-ahead do replay em TFs altos (descoberta E3); zMov/zHist medindo o último dia D1 fechado foi declarado **comportamento oficial** e o GUIA §10 corrigido para descrevê-lo (o guia referencia o cálculo do indicador, não o oposto).
- **Documentação:** `docs/IFM_GUIA.md` cobre 100% da arquitetura da v1.1; `docs/LEITURA.md` (playbook de interpretação) ainda sem entradas.
- **Pesquisas:** `research/2026-07-reatividade-metricas/` em andamento (E0–E7 fechadas; P1–P3 carimbados; próxima etapa: E8 — ver PROGRESS.md da pesquisa). Governança da pesquisa: PLANO.md + TAREFAS.md + PROGRESS.md na pasta da pesquisa; validador `scripts/check_tarefas.py` roda antes de todo commit da pesquisa.
...
4. **Dados brutos de mercado não entram no git** (barras, ticks, exports grandes). Ficam em `data/` dentro da pasta da pesquisa (gitignorados). Só entram scripts, resultados agregados/pequenos e conclusões. *Exceção registrada (2026-07-15, ampliada em 2026-07-16 por decisão de Rhuan): TODO o `data/` das pesquisas é versionado (raw, parquet, sealed, cache) — repo compartilhado com colaborador que precisa dos mesmos dados sem recomputar. O bloco selado segue protegido pelos scripts (recusam `data/sealed/` até a etapa final prevista no PLANO); versionar não muda a disciplina de leitura.*
...
- **Autoridade do projeto (registrado 2026-07-15):** Carlos Eduardo e Rhuan têm autoridade **igual** sobre o projeto e as pesquisas. Portões P1–P4 e auditorias podem ser carimbados por **qualquer um dos dois**; a decisão registrada no `PROGRESS.md` indica **quem** decidiu e vale para ambos.
